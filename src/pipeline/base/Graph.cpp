...
std::vector<Edge> Graph::toEdgeListBFS() const {
    if (m_inputNodePtr == nullptr) return {};
    return checkCycleAndConvertToEdgeList(m_inputNodePtr).second;
}
...
std::pair<bool, std::vector<Edge>> Graph::checkCycleAndConvertToEdgeList(const std::shared_ptr<Node>& inputNodePtr) const {
    ...
    if (inputNodePtr != nullptr) {
        nodeQueue.push(m_inputNodePtr);
        inDegree[m_inputNodePtr] = 0;
    } else {
