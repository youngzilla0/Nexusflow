...
private:
    /**
     * @brief 从 NodeRegistry::NodeState 中提取调度策略所需信息。
     * @param state 当前节点的运行时状态。
     * @return 可供调度策略使用的上下文。
     */
    SchedulingContext BuildSchedulingContext(const NodeRegistry::NodeState& state) const;
     ...
     */
    std::uint64_t ResolveJoinKey(const NodeRegistry::NodeState& state, const Message& message) const;
     ...
     */
    void SubmitNodeTask(const NodeRegistry::NodeStatePtr& state);

    /**
     * @brief 标记节点为 ready，并在必要时触发任务提交。
     * @param state 目标节点的运行时状态。
     */
    void NotifyNodeReady(const NodeRegistry::NodeStatePtr& state);
     ...
     */
    bool HasPendingWork(const NodeRegistry::NodeStatePtr& state) const;
     ...
     */
    void RunNodeTask(const NodeRegistry::NodeStatePtr& state);
     ...
     */
    StepResult RunSourceStep(const NodeRegistry::NodeStatePtr& state);
     ...
     * @return 当前 step 的执行结果。
     */
    StepResult RunOnAnyInputStep(const NodeRegistry::NodeStatePtr& state);
     ...
     * @return 当前 step 的执行结果。
     */
    StepResult RunOnAllInputsStep(const NodeRegistry::NodeStatePtr& state);
     ...
     */
    bool TryPopAnyInput(const NodeRegistry::NodeStatePtr& state, PortMessage& portMessage);
