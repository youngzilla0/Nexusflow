...
std::shared_ptr<Module> ModuleFactory::CreateModule(const std::string& className, const std::string& moduleName,
                                                    const Config& config) {
    return CreateModule(ModuleBuildContext{className, moduleName, config});
}

std::shared_ptr<Module> ModuleFactory::CreateModule(const ModuleBuildContext& context) {
    // Find the creator function associated with the class name.
    auto it = m_creators.find(context.className);
    if (it == m_creators.end()) {
        // The class name was not found in our map of registered creators.
        LOG_ERROR("ModuleFactory Error: Attempted to create a module of unregistered class '{}'.", context.className);
        return nullptr;
    }

    // Call the stored creator lambda function.
    // The lambda will call the constructor: new T(moduleName)
    std::shared_ptr<Module> moduleInst = it->second(context.moduleName);

    if (moduleInst) {
        try {
            moduleInst->Configure(context.config);
        } catch (const std::exception& e) {
            // Log an error if configuration fails
            LOG_ERROR("Failed to configure module '{}' of class '{}': {}", context.moduleName, context.className, e.what());
            return nullptr;
        }
    }
    return moduleInst;
